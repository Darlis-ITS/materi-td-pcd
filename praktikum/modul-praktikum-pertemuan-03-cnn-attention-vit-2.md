...
## Pengantar: Hubungan dengan Praktikum 02

Praktikum 02 membangun fondasi CNN dan Transformer dari komponen dasar. Mahasiswa telah mempelajari citra sebagai tensor, convolution, feature map, pooling, receptive field, residual connection, token, embedding, Query–Key–Value, self-attention, multi-head attention, positional encoding, dan patch tokenization. Konsep tersebut kemudian digunakan untuk membangun `SimpleCNN` dan `TinyImageTransformer` dari nol.

Praktikum 03 melanjutkan fondasi tersebut pada arsitektur modern dan pretrained:

| Praktikum 02 | Praktikum 03 |
|---|---|
| Convolution dan `SimpleCNN` | ResNet-18 pretrained |
| Residual block sederhana | Residual architecture pada ResNet |
| Q, K, V, dan attention matrix | Self-attention pada DeiT |
| Patch 4×4 pada citra 32×32 | Patch 16×16 pada citra 224×224 |
| `TinyImageTransformer` dari nol | DeiT-Tiny pretrained |
| Training seluruh model dari awal | Linear probing dan fine-tuning |
| Feature map dan attention dasar | Analisis representasi CNN dan ViT |
| Evaluasi dasar | Accuracy, macro-F1, parameter, FLOPs, latensi, dan throughput |

Perubahan utama terletak pada tingkat abstraksi. Praktikum 02 menjawab:

> Bagaimana komponen CNN dan Transformer bekerja?

Praktikum 03 menjawab:

> Bagaimana arsitektur CNN dan Vision Transformer modern digunakan, dibandingkan, dan dievaluasi secara metodologis?

Mahasiswa tidak lagi membangun seluruh model dari awal. Model dimuat dengan bobot pretrained ImageNet, classifier disesuaikan dengan CIFAR-10, kemudian model dievaluasi melalui protokol yang terkontrol. Dengan demikian, fokus berpindah dari memahami mekanisme dasar menuju menganalisis pengaruh **arsitektur, inductive bias, pretraining, strategi fine-tuning, dan biaya komputasi**.

Sebelum melanjutkan, pastikan telah memahami:

- fungsi convolution, feature map, dan residual connection;
- perbedaan token dan embedding;
- fungsi Query, Key, dan Value;
- alasan penggunaan positional encoding;
- proses perubahan citra menjadi patch token;
- perbedaan training dari nol, linear probing, dan full fine-tuning;
- fungsi training, validation, dan test set.

Praktikum 02 menjelaskan **bagaimana representasi dibangun**, sedangkan Praktikum 03 menggunakan pemahaman tersebut untuk menilai **kapan dan mengapa suatu representasi lebih sesuai bagi masalah penelitian tertentu**.
